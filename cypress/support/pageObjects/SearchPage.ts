...
class SearchPage {
  ...
  passSearchValueInSearchBox(searchText: string) {
    genericMethods.passTextValueMethod(
      locators.searchPageLocators.searchTextBoxLocator,
      searchText,
    );
  }
  ...
  assertSearchResultsDisplayed() {
    cy.get(locators.searchPageLocators.searchResultsTextLocator)
      .should('be.visible')
      .should((results) => {
  ...
  sortResultsByDateIfNotSortedDefault() {
    cy.get(locators.searchPageLocators.sortByDateButton)
      .should('be.visible')
      .then((element) => {
  ...
  validateArticleUrlLinkStartsAsExpected(websiteUrlText: string) {
    const links = cy
      .get(locators.searchPageLocators.resultsDisplayLinkLocator)
      .should('be.visible');
